...
public class RequestLoggingAspect {
	...
	// 포인트컷 설정
	@Pointcut("within(com.gdu.app01.controller..*)")  // controller 패키지 아래 모든 패키지(..)의 모든 클래스(*)
	...
	public Object executeLogging(ProceedingJoinPoint joinPoint) throws Throwable {  // @Around는 반드시 ProceedingJoinPoint joinPoint 선언해야 함.
		...
		HttpServletRequest request = ((ServletRequestAttributes)RequestContextHolder.currentRequestAttributes()).getRequest();  // 현재 모든 요청에 관련된 객체 중에서 request를 가져와라
		...
		if(map.isEmpty()) {
			...
			for(Map.Entry<String, String[]> entry : map.entrySet()) {
				params += "[" + entry.getKey() + "=" + String.format("%s", (Object[])entry.getValue()) + "]"; // %s = 문자 출력, %d = 정수 출력, %f = 실수 출력
			}
		}
