...
-- 1) EMPLOYEES 테이블의 사원정보를 연봉이 높은 순으로 조회하기
--      연봉 순위를 함께 조회하기
SELECT 
        EMPLOYEE_ID, FIRST_NAME, LAST_NAME, SALARY,
        RANK() OVER(ORDER BY SALARY DESC) AS 연봉순위       -- RANK()로 자동 정렬처리까지 됨.
FROM
        EMPLOYEES;
    
-- 2) EMPLOYEES 테이블의 사원정보를 입사 순으로 조회하기
--      먼저 입사한 사원이 1등
...
-- JOIN 없이 EMPLOYEES 테이블만 이용하여 EMPLOYEES_ID, DEPARTMENT_NAME 조회하기 (JOIN은 성능저하의 주요 요인)
SELECT
            EMPLOYEE_ID
