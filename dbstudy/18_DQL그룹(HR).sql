...
-- 위에 쿼리가 더 좋은 쿼리 (WHERE절 안에서 1차분류를 하고 그룹화가 진행되어서)
...
-- 각 부서에 소속된 사원 수는 GROUP BY 이후에만 알 수 있기 때문에 WHERE절로 처리가 불가능
SELECT
            DEPARTMENT_ID
        ,   COUNT(*) AS 부서별사원수
        ,   FLOOR(AVG(SALARY)) AS 부서별연봉평균
...
SELECT
            SUBSTR(DEPARTMENT_NAME, 1 , 2), COUNT(*)
FROM
            DEPARTMENTS
GROUP BY
