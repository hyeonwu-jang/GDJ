...
SELECT E.EMPLOYEE_ID, E.LAST_NAME, E.JOB_ID
  FROM JOBS J INNER JOIN EMPLOYEES E
    ON J.JOB_ID = E.JOB_ID INNER JOIN JOB_HISTORY JH
...
SELECT D.DEPARTMENT_NAME 부서명, COUNT(*) 사원수, TRUNC(AVG(SALARY)) 평균연봉 
  FROM DEPARTMENTS D INNER JOIN EMPLOYEES E
    ON D.DEPARTMENT_ID = E.DEPARTMENT_ID
 GROUP BY D.DEPARTMENT_ID, D.DEPARTMENT_NAME                                -- NAME으로 그룹화할 경우 중복그룹이 존재할 수 있어 ID와 NAME을 그룹화로 지정하여 NAME으로만 조회
 ORDER BY 사원수;
...
SELECT E.EMPLOYEE_ID, E.LAST_NAME, D.DEPARTMENT_NAME, L.CITY, C.COUNTRY_NAME
  FROM COUNTRIES C INNER JOIN LOCATIONS L                                   -- 가장 수가 적은 테이블을 드라이브 테이블로
...
-- WHERE D.DEPARTMENT_ID IS NOT NULL;                                       -- 굳이 조건을 주지 않아도 조회가 안됨.
...
SELECT D.DEPARTMENT_ID, D.DEPARTMENT_NAME, COUNT(E.DEPARTMENT_ID) AS 사원수            -- COUNT(*) 사용하면 비어있는 행의 수를 COUNT하기 때문에 1이 출력됨!!
...
SELECT
         E.EMPLOYEE_ID 사원번호  -- 사원번호
       , E.LAST_NAME 사원이름    -- 사원이름
       , E.HIRE_DATE 사원입사일  -- 사원입사일
       , M.LAST_NAME 상사이름    -- 상사이름
       , M.HIRE_DATE 상사입사일  -- 상사입사일
  FROM
