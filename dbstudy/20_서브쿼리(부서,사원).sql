...
SELECT EMP_NO, NAME, DEPART, GENDER, POSITION, HIRE_DATE, SALARY
  FROM EMPLOYEE
 WHERE (SYSDATE - HIRE_DATE) >= (SELECT AVG(SYSDATE-HIRE_DATE)              -- 날짜는 산술연산이 가능하다
                                   FROM EMPLOYEE);
...
SELECT EMP_NO, NAME, DEPART, GENDER, POSITION, HIRE_DATE, SALARY
  FROM EMPLOYEE
 WHERE POSITION IN (SELECT POSITION     -- 다중 행 서브쿼리의 동등비교는 IN 연산으로 수행해야 함
                      FROM EMPLOYEE
                     WHERE DEPART = 2); -- DEPART가 PK/UNIQUE가 아니기 때문에 다중 행 서브쿼리임(DEPART는 FK)
...
SELECT EMP_NO, NAME, DEPART, GENDER, POSITION, HIRE_DATE, SALARY
  FROM EMPLOYEE
 WHERE DEPART IN (SELECT DEPT_NO                                    -- DEPART와 DEPT_NO는 참조관계이므로 조회가 가능
                    FROM DEPARTMENT
                   WHERE DEPT_NAME = '영업부');
...
--  어떤 급여이든 하나의 급여보다 많이 받으면 조회하기
SELECT EMP_NO, NAME, DEPART, GENDER, POSITION, HIRE_DATE, SALARY
  FROM EMPLOYEE
 WHERE SALARY > ANY(SELECT SALARY           
                      FROM EMPLOYEE
                     WHERE DEPART = 1);     -- DEPART가 PK/UNIQUE가 아니기 때문에 다중 행 서브쿼리임(DEPART는 FK)
...
-- 따라서 최대급여 50000000보다 만족하면 되는 상황임
SELECT EMP_NO, NAME, DEPART, GENDER, POSITION, HIRE_DATE, SALARY
  FROM EMPLOYEE
 WHERE SALARY > (SELECT MAX(SALARY)           
                   FROM EMPLOYEE
...
SELECT 
        ...
        , (SELECT D.DEPT_NAME
             FROM DEPARTMENT D
            WHERE D.DEPT_NO = E.DEPART          -- 아무 데이터나 가져오면 안되기 때문에 PK = FK 연결(조인)
              AND D.DEPT_NO = 1)
  FROM
          EMPLOYEE E;
...
SELECT EMP_NO, NAME
  FROM EMPLOYEE
 WHERE ROWID = 'AAAFFKAABAAALDBAAC';
...
-- ROWNUM을 1 이외의 범위를 조건으로 사용하는 방법 (실행 순서에 따른 인라인뷰 먼저 생성)
-- ROWNUM에 별명을 지정하고 해당 별명을 사용하면 됨
-- 3 : SELECT
-- 1 :   FROM (ROWNUM의 별명 지정하기)
-- 2:   WHERE (ROWNUM의 별명 사용하기)
SELECT A.EMP_NO, A.NAME
  FROM (SELECT ROWNUM AS ROW_NUM, EMP_NO, NAME          -- 인라인뷰 생성 + ROWNUM칼럼 별명 지정
          FROM EMPLOYEE) A
 WHERE A.ROW_NUM = 2;
...
-- 2) 정렬과 ROWNUM 이용 (1 실행순서, 2 정렬, 3 ROWNUM)
--      (1) 연봉의 내림차순 정렬을 수행(가장 높은 연봉이 1번째 행이 됨)
--      (2) 정렬 결과에서 ROWNUM = 1인 행을 조회
SELECT ROWNUM, A.EMP_NO, A.NAME, A.SALARY
  FROM (SELECT EMP_NO, NAME, SALARY     -- (실행 순서에 따라 DESC 정렬을 위한 인라인뷰 먼저 생성)
          FROM EMPLOYEE
...
SELECT A.EMP_NO, A.NAME, A.DEPART, A.POSITION, A.GENDER, A.HIRE_DATE, A.SALARY
  FROM (SELECT ROW_NUMBER() OVER(ORDER BY HIRE_DATE) AS ROW_NUM, EMP_NO, NAME, DEPART, POSITION, GENDER, HIRE_DATE, SALARY
          FROM EMPLOYEE) A
...
SELECT CONSTRAINT_NAME FROM USER_CONSTRAINTS WHERE TABLE_NAME = 'EMPLOYEE';   -- 제약조건 O
SELECT CONSTRAINT_NAME FROM USER_CONSTRAINTS WHERE TABLE_NAME = 'EMPLOYEE2';  -- NOT NULL 제외 제약조건 X
