...
BEGIN
    SELECT 
           FIRST_NAME, LAST_NAME, SALARY            -- 순서대로
      INTO                                          --    ||
           F_NAME, L_NAME, V_SALARY                 -- 순서대로
      FROM 
           EMPLOYEES
     WHERE 
           EMPLOYEE_ID = 100;
...
DECLARE
        SAL EMPLOYEES.SALARY%TYPE;      -- 참조할 변수인 SALARY 변수 SAL 등록   
        RES VARCHAR2(20 BYTE);
BEGIN
    SELECT  SALARY                      -- SELECT로 SALARY를 검색해서
    INTO    SAL                         -- INTO로 SAL에 값을 대입
    FROM    EMPLOYEES                   
    WHERE   EMPLOYEE_ID = 150;          -- 사번이 150번인 데이터
            
    IF SAL >= 15000 THEN                -- 이후에 IF절 시작(WHERE에서 불가능)
       RES := '고연봉';
...
DECLARE
    -- 참조 변수 선언
    EMP_NO EMPLOYEES.EMPLOYEE_ID%TYPE;      -- 루프돌릴 번호
    
    -- 레코드 변수 정의
    TYPE NAME_TYPE IS RECORD(
        FNAME EMPLOYEES.FIRST_NAME%TYPE,    -- 레코드 변수 정의
        LNAME EMPLOYEES.LAST_NAME%TYPE      -- (레코드에 입력될 칼럼의 타입을 참조값으로 정의)
    );
...
DECLARE
    N NUMBER;
    MODULAR NUMBER; -- 2로 나눈 나머지 값을 저장
...
DECLARE
    -- 참조 변수 선언
    EMP_NO EMPLOYEES.EMPLOYEE_ID%TYPE;      -- 타입 선언
    EMP_SAL EMPLOYEES.SALARY%TYPE;          -- 타입 선언
...
--     WHERE EMPLOYEE_ID = 0;     -- 존재하지 않는 사원 조회
      WHERE DEPARTMENT_ID = 50;   -- 변수에 저장할 데이터가 너무 많아서 에러 발생  
EXCEPTION
