...
-- 프로시저 PROC4 호출 -----------------------------------------------------------------
DECLARE
    FNAME EMPLOYEES.FIRST_NAME%TYPE; -- 출력 파라미터로 사용할 변수를 (프로시저 IS가 아닌), DECLARE에서 선언
BEGIN
    PROC4(FNAME); -- PLSQL(프로그래밍) 내부에서 프로시저를 호출할 땐 EXECUTE 생략
    DBMS_OUTPUT.PUT_LINE(FNAME);
END;

-- 프로시저 PROC5 정의 -----------------------------------------------------------------   참조변수, OUT 파라미터, 예외
...
EXCEPTION
    WHEN OTHERS THEN -- 모든 예외를 처리함( WHEN NO_DATE_FOUND THEN 가능 )
        FNAME := 'NoName';
END PROC5;
...
-- 연습1. 입력 파라미터에 사원번호 전달, 출력 파라미터에 FIRST_NAME 반환받기 -------------- 참조변수, (IN, OUT) 파라미터, 예외
