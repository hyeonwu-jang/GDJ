...
SELECT EMPLOYEE_ID, FIRST_NAME, JOB_ID, DEPARTMENT_ID
  FROM EMPLOYEES
 WHERE (JOB_ID, DEPARTMENT_ID) = (SELECT JOB_ID, DEPARTMENT_ID          -- 칼럼은 2개, 행은 1개(EMPLOYEE_ID = 115) => 단일 행 서브쿼리
                                    FROM EMPLOYEES
...
-- 서브쿼리 : MANAGER_ID의 중복제거 결과(MANAGER의 번호 모아두기)
...
-- 가장 많은 사원이 근무중인 부서 : 부서별 사원수를 구해야 알 수 있음

-- 조건 : 근무 중인 사원 수 = 최대 사원수
-- HAVING절? WHERE절?
...
-- 인라인뷰 B : 연봉순으로 정렬된 테이블에 행 번호를 부착시켜둔 테이블
