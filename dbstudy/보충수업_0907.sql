...
SELECT C.CUSTOMER_NAME AS 고객명, SUM(B.PRICE * O.AMOUNT) AS 총구매액
 ...
 WHERE B.BOOK_ID = O.BOOK_ID
   AND C.CUSTOMER_ID = O.CUSTOMER_ID
 GROUP BY C.CUSTOMER_ID, C.CUSTOMER_NAME;   -- NAME으로만 그룹을 지었을 경우 중복이름이 존재할 수 있다. 그래서 ID도 같이 그룹을 지어준다.
...
SELECT 
      ...
      , COUNT(O.ORDER_ID) AS 구매횟수             -- COUNT에는 고객정보가 포함되면 안 됨. 이유는 LEFT OUTER JOIN이라 고객은 모두 조회되기 때문이다.
...
-- 박지성  3
SELECT C.CUSTOMER_NAME AS 고객명
     , COUNT(DISTINCT B.PUBLISHER) AS 출판사수              -- DISTINCT 사용하는 이유는 
  FROM CUSTOMER C INNER JOIN ORDERS O
...
SELECT C.CUSTOMER_NAME AS 고객명
  FROM CUSTOMER C LEFT OUTER JOIN ORDERS O
    ON C.CUSTOMER_ID = O.CUSTOMER_ID
 GROUP BY C.CUSTOMER_ID, C.CUSTOMER_NAME
HAVING COUNT(O.ORDER_ID) = 0;                               -- HAVING에 들어가는 조건은 대체로 집계함수가 포함된 조건임.
--              ||
SELECT C.CUSTOMER_NAME AS 고객명
  FROM CUSTOMER C LEFT OUTER JOIN ORDERS O
    ON C.CUSTOMER_ID = O.CUSTOMER_ID
