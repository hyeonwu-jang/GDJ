...
public class Ex04_Calendar {
	...
	public static void main(String[] args) {
		...
		Calendar cal = Calendar.getInstance(); // 객체 cal은 현재(실행하는 시점) 날짜와 시간으로 구성
		
		// 년, 월, 일, 요일
		int year = cal.get(Calendar.YEAR); 			// int year = cal.get(1);
		int month = cal.get(Calendar.MONTH) + 1; 	// 월 반환값 : 0 ~ 11 (주의가 필요함)
		int day = cal.get(Calendar.DAY_OF_MONTH);
		int weekNo = cal.get(Calendar.DAY_OF_WEEK); // 요일 번호 : 일(1), 월(2), ... 토(7)
