...
public class Ex01_String {
	...
	// equalsIgnoreCase
		...
		boolean result3 = str5.equalsIgnoreCase(str6);  // equalsIgnoreCase = 대소문자 무시 동등 비교
		System.out.println(result3); 					// true
		
	// 3. 문자열 길이 변환  <--------- 띄어쓰기를 하는 경우, 글자수에 포함.
	// length
		String name = "장현우";
	...
	// 5. 문자열의 일부 문자열(String)을 반환  <---------------- 많이 활용됨
	// 	1) substring(begin) : 인덱스 begin(포함)부터 끝까지 반환
	// 	2) substring(begin, end) : 인덱스 begin(포함)부터 인덱스 end(불포함)까지
		System.out.println(name.substring(0, 1));
		System.out.println(name.substring(1));
		
	// 6. 특정 문자열을 찾아서 해당 인덱스의 첫번째 인덱스(int)를 반환
	// 	1) indexOf(문자열)
	//		(1) 발견된 첫 번재 문자열의 인덱스를 반환 (ex.우영우 => 0 반환)
	...
	// startWith(문자열)
		if(name.startsWith("장")) {
			System.out.println("장씨입니다.");
		} else {
			System.out.println("장씨가 아닙니다.");
		}
	...
	// 13. 빈 문자열인지 여부를 검색한 뒤 boolean(true, false) 반환
		String id = " ";
		if(id.isEmpty()) {
	...
	// 13-2. 빈 문자열인지 여부를 검색한 뒤 boolean(true, false) 반환 + trim
		id = " ";
		if(id.trim().isEmpty()) {
			System.out.println("빈 문자열");
		}	else {
		...
		extName = fullName.substring(idxOfDot);
