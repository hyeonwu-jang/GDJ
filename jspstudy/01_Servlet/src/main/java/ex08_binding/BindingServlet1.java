...
public class BindingServlet1 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// HttpServletRequest	// 매개변수로 선언되어 있음
		...
		// <a href> 링크 이동
		// PrintWriter out = response.getWriter();											binding1 -> binding2
		// out.println("<a href=\"/01_Servlet/BindingServlet2\">이동</a>"); out.close();	a = 1, b = 2, c = null
		 
		// 포워드
		// 1. request를 전달함
		// 2. 컨텍스트 내부 이동이므로 경로 작성 시 컨텍스트 패스는 작성하지 않음           binding1 -> binding2
		// request.getRequestDispatcher("/BindingServlet2").forward(request, response);		a = 1, b = 2, c = 3
		
		// 리다이렉트
		// 1. request를 전달하지 않음
		// 2. 클라이언트 -> 서버로 이동하므로 컨텍스트 패스를 작성해야 함					binding1 -> binding2
		response.sendRedirect("/01_Servlet/BindingServlet2");							 // a = 1, b = 2, c = null
