...
<%
	...
	// 2. cart 속성이 없으면 새로 만들어서 저장한다.
																								// List, Map이 사용된 이유 = 배열로 가져오기 위해서
	List<Map<String, Object>> cart = (List<Map<String,Object>>)session.getAttribute("cart");    // 2. 카트에 물건이 있을 때는 세션에 담긴 기존 카트를 사용한다.
	
	if(cart == null) {
		cart = new ArrayList<>();  																// 1. 카트에 물건이 없을 때는 새로운 카트를 생성한다.
