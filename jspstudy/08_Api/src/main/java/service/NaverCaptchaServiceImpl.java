...
public class NaverCaptchaServiceImpl implements NaverCaptchaService {
	...
	// field
	private final String CLIENT_ID = "4H16u9bbdy10BBd8HKjl";
	private final String CLIENT_SECRET = "FSJHue2eMD";
	...
	public String getCaptchaKey() {
		...
		try {
			...
			BufferedReader reader = null;		// text 기반인 json로 받아오기 떄문에 BufferedReader사용
			if(con.getResponseCode() == 200) {  // 200 : HttpURLConnecion.HTTP_OK
			...
			// 네이버 API서버가 보낸 데이터 확인 빛 반환
			JSONObject obj = new JSONObject(sb.toString());		// JSONObject에 key, 값 저장 {"key":"JrtStof3pzlpWEyk"} 
	...
	public Map<String, String> getCaptchaImage(HttpServletRequest request, String key) {
		...
		try {
			...
			// byte 기반인 jpeg로 받아오기 떄문에 inputstream사용
			...
			if(con.getResponseCode() == 200) {  // 200 : HttpURLConnecion.HTTP_OK
				...
				// login.jsp로 전달할 데이터(캡차이미지 경로 + 파일명 + 캡차키) -- 전달하지 않으면 소멸
			...
			// 응답이 실패하면 텍스트 형식으로 응답 -> BufferdReader 사용
			else {
