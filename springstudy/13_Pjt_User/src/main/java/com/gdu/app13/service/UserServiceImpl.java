...
public class UserServiceImpl implements UserService {
	...
	public Map<String, Object> sendAuthCode(String email) {
		...
		// 인증코드 만들기
		String authCode = securityUtil.getAuthCode(6);  // String authCode = securityUtil.generateRandomString(6);  // dependency
	...
	public void join(HttpServletRequest request, HttpServletResponse response) {
		...
		if(!location.isEmpty() && promotion.isEmpty()) {
			...
			agreeCode = 2;  // 필수 + 프로모션
		} else if(!location.isEmpty() && !promotion.isEmpty()) {  // 나중을 대비해서 else if 사용
		...
		try {
			...
			if(result > 0) {
				...
				// 가입 후 로그인 처리를 위해서 session에 로그인 된 사용자 정보를 올려둠
	...
	public void login(HttpServletRequest request, HttpServletResponse response) {
		...
		if(loginUser != null) {
			...
			// 로그인 유지 처리는 keepLogin 메소드가 따로 처리함 (후작업 메소드 호출)
	...
	public void keepLogin(HttpServletRequest request, HttpServletResponse response) {
		...
		*/
		...
		// 로그인 유지를 체크한 경우 (체크박스는 체크하지 않는 경우 null 반환)
		if(keepLogin != null) {
			...
			UserDTO user = UserDTO.builder()
					.id(id)
					.sessionId(sessionId)
					.sessionLimitDate(new Date(System.currentTimeMillis() + 1000 * 60 * 60 * 24 * 15))  // 현재 타임 스탬프 + 15일에 해당하는 타임 스탬프(초 단위에 1000을 곱한 값)
					.build();
		...
		// 로그인 유지를 체크하지 않은 경우
			...
			cookie.setMaxAge(0);  // 쿠키 유지시간이 0이면 삭제를 의미함
			cookie.setPath(request.getContextPath());
			response.addCookie(cookie);
		}
	...
	public String getNaverLoginApiURL(HttpServletRequest request) {
		...
		String apiURL = null;
		try {
			String clientId = "4H16u9bbdy10BBd8HKjl";//애플리케이션 클라이언트 아이디값";
