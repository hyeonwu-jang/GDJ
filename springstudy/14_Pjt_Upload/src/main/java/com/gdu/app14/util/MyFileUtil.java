...
public class MyFileUtil {
	...
	// 파일명 : UUID값을 사용 (Universally Unique ID)   장점 - 1) unique, 2) 인코딩
	...
	public String getFilename(String filename) {
		...
		if(filename.endsWith("tar.gz")) {
			...
			String[] arr = filename.split("\\.");   // 정규식에서 .(마침표)인식 : \. 또는 [.] = 모든 문자를 의미  / split(정규식) 
	...
	public String getTodayPath() {
		...
		String sep = Matcher.quoteReplacement(File.separator);  // (운영체제에 따라 / 또는 \)
