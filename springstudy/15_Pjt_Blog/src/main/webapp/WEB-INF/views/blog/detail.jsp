...
<div>
	...
	<div>
		<span>▷ 작성일 <fmt:formatDate value="${blog.createDate}" pattern="yyyy. M. dd  hh:mm:ss"/> </span>
		&nbsp;&nbsp;&nbsp;
		<span>▷ 수정일 <fmt:formatDate value="${blog.modifyDate}" pattern="yyyy. M. dd  hh:mm:ss"/> </span>
	</div>
		...
		<form id="frm_add_comment">
			<div class="add_comment">
				<div class="add_comment_input">
					<input type="text" id="content" name="content" placeholder="댓글을 작성하려면 로그인 해주세요.">
				</div>
	...
	<script>
				...
				$.ajax({
					...
					success: function(resData) {  // resData = {"isAdd", true}
						if(resData.isAdd) {
							alert('댓글이 등록되었습니다.');
							$('#content').val('');
							fn_commentList();   // 댓글 목록 가져와서 뿌리는 함수 + 페이징
				...
				success: function(resData) {
					...
					*/
					...
					$.each(resData.commentList, function(i, comment){
						...
						div += '<input type="hidden" name="groupNo" value="' + comment.commentNo + '">';  // groupNo, commentNo 둘 다 가능
		...
		function fn_changePage() {  // 동적으로 생성된 요소라 일반 클릭이벤트로는 적용이 불가함.
		...
		function fn_removeComment() {  // 동적 생성
				...
				if(confirm('삭제된 댓글은 복구할 수 없습니다. 댓글을 삭제할까요?')) {
					$.ajax({
						...
						success: function(resData) {  // resData = {"isRemove": true}
							if(resData.isRemove) {
								alert('댓글이 삭제되었습니다.');
								fn_commentList();   // 목록 가져와서 뿌리는 함수
								fn_commentCount();  // 댓글목록 개수 갱신 함수
							}
						}
		...
		function fn_addReply() {
			$(document).on('click', '.btn_reply_add', function() {
				...
				$.ajax({
					...
					data: $(this).closest('.frm_reply').serialize(),  // <-- 상대위치 지정해야함. closest 가장가까운 요소~ / $('.frm_reply').serialize() - 사용 불가
					dataType: 'json',
					success: function(resData) {  // resData = {"isAdd", true}
						if(resData.isAdd) {
							alert('답글이 등록되었습니다.');
