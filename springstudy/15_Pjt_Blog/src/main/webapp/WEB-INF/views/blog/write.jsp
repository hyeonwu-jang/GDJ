...
<script>
	...
	$(document).ready(function() {
		...
		$('#content').summernote({
			...
			callbacks: {
				// 내용에 이미지가 첨부되는 경우 용량이 과도하게 증가해서 아래 처리를 기본으로 해야함.
				...
				onImageUpload: function(files) {
					...
					$.ajax({
						...
						success: function(resData) {
							...
							$('#content').summernote('insertImage', resData.src);  // src는 경로를 받아옴!
							
							/*
								src = ${contextPath}/load/image/aaa.jpg 값이 넘어온 경우
								summernote는 
								<img src=${contextPath}/load/image/aaa.jpg"> 태그를 만든다.   <-- 경로가 아닌 매핑값(servlet-context.xml 참고)
								...
								스프링에서 정적 자원 표시하는 방법은 servlet-context.xml에 있다.
								이미지(정적자원) 의 mapping과 location을 servlet-context.xml에 작성해야 한다.
							*/
						}
					}); // ajax
				} // onImageUpload
